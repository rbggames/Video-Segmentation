...
void Evaluator::evaluateSegments(Shape** actualShapes, int numShapes, TrackedObject** trackedShapes, int numTrackedShapes)
{
	for (int i = 0; i < numShapes; i++) {
		int bestMatchIndex=-1;
		double bestMatchValue=-1;
		for (int j = 0; j < numTrackedShapes; j++) {
			double similarity = evaluateSegmentsSimalarity(actualShapes[i], trackedShapes[j]);
